...
export type NewsTeaserSectionLabels = {
  eventBadge?: string;
  newsBadge?: string;
  readDetailsFallback?: string;
  openFallback?: string;
  emptyMessage?: string;
};
...
type NewsTeaserSectionProps = {
  ...
  labels?: NewsTeaserSectionLabels;
};
...
const NewsTeaserSection = ({
  ...
  const {
    eventBadge = "Event",
    newsBadge = "News",
    readDetailsFallback = "Read details",
    openFallback = "Open",
          ...
          <Card className="border-border/80">
            <CardHeader>
              <div className="mb-2 inline-flex w-fit items-center gap-2 rounded-full border px-2.5 py-1 text-xs text-muted-foreground">
                ...
                <span>{featured.type === "news" ? newsBadge : eventBadge}</span>
              </div>
            ...
            {featured.summary ? (
              <CardContent className="pt-0">
                ...
                <Button asChild variant="link" className="mt-3 px-0">
                  <Link to={featured.href}>{featured.ctaLabel ?? readDetailsFallback}</Link>
                </Button>
              </CardContent>
            ) : null}
          </Card>
                ...
                <CardContent className="flex items-start justify-between gap-3 p-4">
                  <div className="min-w-0">
                    <p className="text-xs uppercase tracking-wide text-muted-foreground">
                      {entry.type === "news" ? newsBadge : eventBadge}
                    </p>
                  ...
                  <Button asChild variant="ghost" size="sm">
                    <Link to={entry.href}>{entry.ctaLabel ?? openFallback}</Link>
                  </Button>
                </CardContent>
              </Card>
            ))}
