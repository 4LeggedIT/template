...
**Header dropdown overflow — fleet follow-ups (2026-08-27).** `getDesktopDropdownPositionClass` centered a lone dropdown panel under its trigger; because the desktop nav is right-aligned, that pushed the 24rem panel past the container and made the document scroll sideways by ~28px at ~1100px viewports (both EN and ES — the Spanish labels made it noticeable but were not the cause). Fixed here and in `internal-tools/4leggedit-websites/templates-sources/patterns/SiteHeader.tsx` by anchoring the last dropdown `right-0` so it opens inward. A fleet-wide sweep found no other site currently overflowing, but three follow-ups remain:

1. **`4leggedit` and `pet-care-paths-site` hardcode `left-1/2 -translate-x-1/2` on every dropdown panel** (`src/components/Header.tsx` / `src/components/layout/Header.tsx`) with no per-position logic at all — not even the `right-0` anchoring the last panel gets here. Both measure clean today only incidentally (4leggedIT's nav is left-aligned with 220px panels; pet-care-paths has a right-aligned nav with 192px panels whose rightmost edge lands at 1005/1100 — ~95px of headroom). One longer nav label or a wider panel reintroduces the bug. Give both the same right-anchoring.
2. **Dead `patterns/SiteHeader.tsx` copies** in `enders-wish-foundation` and `themisfitranch` still carry the pre-fix line. Neither is rendered (both sites render their own `components/Header.tsx` / `layout/Header.tsx`), so they are harmless, but they keep surfacing in fleet greps as false positives. Delete them or sync them.
3. **`enders-wish-foundation/src/components/layout/Header.tsx` carries its own near-duplicate of the helper** (keyed by `label` rather than index). It is immune today (4 dropdowns, so the `<= 1` branch never fires) but it is a second copy that will drift from the module source.

(2026-08-16: the AST-based `check-internal-links.mjs` rewrite briefly surfaced two issues, both resolved same day — a genuine pre-existing bug where `CommunityPartnersStandardPage.tsx`/`SupporterRecognitionStandardPage.tsx`/`ImpactStatsStandardPage.tsx` pointed illustrative example CTAs at `/contact`/`/donate`, routes this repo doesn't have, now repointed at real destinations (`/standards`, `/standards/paypal-donate`); and a checker false-positive on the `` `${basePath}/${slug}` `` idiom used by `BlogSection`/`BlogPostDetail`'s `postBasePath` prop, fixed in the checker itself — see `internal-link-checker-governance.md`.)
