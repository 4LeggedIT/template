...
### i18n
`src/i18n/index.ts` (i18next + react-i18next) covers the site's own chrome only — nav, footer, home page, error pages — via `src/locales/{en,es}/*.json`. Each pattern's own "Standard" prose stays English-only (developer-facing documentation); see `src/lib/localized-content.ts` for the separate `LocalizedText`/`resolveText` helper that real bilingual client sites use for their own content data.
