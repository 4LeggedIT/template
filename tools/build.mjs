...
async function writeRobotsTxt(siteOrigin) {
  ...
  const lines = ["User-agent: *", "Allow: /", "", `Sitemap: ${siteOrigin}/sitemap.xml`, ""];
  await fs.writeFile(robotsPath, lines.join("\n"), "utf-8");
}

function extractPageMeta(headHtml) {
  if (!headHtml) return {};
  const title = headHtml.match(/<title[^>]*>([^<]+)<\/title>/i)?.[1] ?? null;
  const descTag = headHtml.match(/<meta[^>]+name=["']description["'][^>]*>/i)?.[0] ?? null;
  const description = descTag?.match(/content=["']([^"']*?)["']/i)?.[1] ?? null;
  const canonicalTag = headHtml.match(/<link[^>]+rel=["']canonical["'][^>]*>/i)?.[0] ?? null;
  const canonicalUrl = canonicalTag?.match(/href=["']([^"']*?)["']/i)?.[1] ?? null;
  return { title, description, canonicalUrl };
}

function injectHead(template, headHtml) {
  if (!headHtml) return template;
  let result = template.replace("</head>", `${headHtml}\n</head>`);

  const { title, description, canonicalUrl } = extractPageMeta(headHtml);

  if (title) {
    // `title` was extracted from already-serialized HTML (react-helmet-async), so
    // entities like `&amp;` are already escaped — only quotes need escaping here
    // for safe embedding in a double-quoted attribute. Re-escaping `&` would
    // double-encode (`&amp;` -> `&amp;amp;`).
