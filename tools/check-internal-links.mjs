...
const extractIdsFromFile = (content) => {
  ...
  // Data-driven anchors (e.g. `id={section.id}` in JSX) declare their id as an
  // object-literal property in a data file instead of a JSX attribute -- catch
  // `id: "some-id"` too so those resolve as known hash targets.
  const idObjectProperty = /\bid\s*:\s*["']([A-Za-z][\w-]*)["']/g;

  for (const re of [idLiteral, idBraceLiteral, idObjectProperty]) {
    let match;
